...
#define FORCE(STATE) SET(PORTB,_BV(PC6),STATE)
#define FORCE_DDR(DIR) SET(DDRB,_BV(PC6),DIR)
#define FORCE_IN GET(PINB,_BV(PC6))

#define TRIGGER_BTN(STATE) SET(PORTC,_BV(PC7),STATE)
#define TRIGGER_BTN_DDR(DIR) SET(DDRC,_BV(PC7),DIR)
#define TRIGGER_BTN_IN GET(PINC,_BV(PC7))

#define ENABLE(STATE) SET(PORTB,_BV(PB2),STATE)
#define ENABLE_DDR(DIR) SET(DDRB,_BV(PB2),DIR)
#define ENABLE_IN GET(PINB,_BV(PB2))

#define EDGE_SWITCH(STATE) SET(PORTC,_BV(PB1),STATE)
#define EDGE_SWITCH_DDR(DIR) SET(DDRC,_BV(PB1),DIR)
#define EDGE_SWITCH_IN GET(PINC,_BV(PB1))

#define LED1(STATE) SET(PORTC,_BV(PC0),~STATE) // Optional LED GPIO
#define LED1_DDR(DIR) SET(DDRC,_BV(PC0),DIR)
#define LED1_IN GET(PINC,_BV(PC0))

#define LED2(STATE) SET(PORTC,_BV(PC0),~STATE) // Optional LED GPIO
#define LED2_DDR(DIR) SET(DDRC,_BV(PC0),DIR)
#define LED2_IN GET(PINC,_BV(PC0))

#define LED3(STATE) SET(PORTC,_BV(PC1),~STATE) // Optional LED GPIO
#define LED3_DDR(DIR) SET(DDRC,_BV(PC1),DIR)
#define LED3_IN GET(PINC,_BV(PC1))

#define LED4(STATE) SET(PORTC,_BV(PC0),~STATE) // Optional LED GPIO
#define LED4_DDR(DIR) SET(DDRC,_BV(PC0),DIR)
#define LED4_IN GET(PINC,_BV(PC0))
